...
class MLPmodel(nn.Module):
    def __init__(self, a, b, c, d, e, f, g):
        ...
        self.hidden7 = nn.Sequential(
            nn.Linear(f, g), nn.BatchNorm1d(g), nn.ReLU())

        self.regression = nn.Sequential(nn.Linear(g, 1), nn.ReLU())
...
# Source data
workbook = '6609-OHads.xlsx'  # Input data excel
sheet = str("6609-OHads")  # Input data excel sheet
...
folder_name = f"Source_4_{len(label_s)}_OHads_{label_t_non_null_count}_{len(label_t)}" + \
