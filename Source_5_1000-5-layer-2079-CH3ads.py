...
class MLPmodel(nn.Module):
    def __init__(self, a, b, c, d, e):
        ...
        self.regression = nn.Sequential(nn.Linear(e, 1), nn.ReLU())
...
# Source data
workbook = '2079-CH3ads.xlsx'  # Input data excel
sheet = str("2079-CH3ads")  # Input data excel sheet
...
folder_name = f"Source_5_{len(label_s)}_CH3ads_{label_t_non_null_count}_{len(label_t)}" + \
...
with torch.no_grad():
    h_s_train_initial = mlp.hidden5(mlp.hidden4(mlp.hidden3(
               mlp.hidden2(mlp.hidden1(torch.from_numpy(
               np.array(trainX_s).astype(np.float32))))))).numpy()
    h_s_test_initial = mlp.hidden5(mlp.hidden4(mlp.hidden3(
              mlp.hidden2(mlp.hidden1(torch.from_numpy(
              np.array(testX_s).astype(np.float32))))))).numpy()

    h_t_train_initial = mlp.hidden5(mlp.hidden4(mlp.hidden3(
               mlp.hidden2(mlp.hidden1(torch.from_numpy(
               np.array(trainX_t).astype(np.float32))))))).numpy()
    h_t_test_initial = mlp.hidden5(mlp.hidden4(mlp.hidden3(
              mlp.hidden2(mlp.hidden1(torch.from_numpy(
...
with torch.no_grad():
    h_s = mlp.hidden5(mlp.hidden4(mlp.hidden3(
        mlp.hidden2(mlp.hidden1(torch.from_numpy(
        dataX_source.astype(np.float32))))))).numpy()

    h_t = mlp.hidden5(mlp.hidden4(mlp.hidden3(
        mlp.hidden2(mlp.hidden1(torch.from_numpy(
