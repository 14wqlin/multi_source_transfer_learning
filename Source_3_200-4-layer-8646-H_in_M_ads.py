...
def plot_results(train_true_list, train_pred_list, test_true_list, test_pred_list, folder_name):
    ...
    plt.xlabel('DFT Calculated $E_{\mathrm{ads}}$(H@M)', fontsize=12)
    plt.ylabel('ML Predicted $E_{\mathrm{ads}}$(H@M)', fontsize=12)
...
# Source data
workbook = '8646-H_in_M_ads.xlsx'  # Input data excel
sheet = str("8646-H_in_M_ads")  # Input data excel sheet
...
folder_name = f"Source_3_{len(label_s)}_H_in_M_ads_{label_t_non_null_count}_{len(label_t)}" + \
