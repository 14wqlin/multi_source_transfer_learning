...
class MLPmodel(nn.Module):
    def __init__(self, a, b, c, d, e, f):
        ...
        self.hidden5 = nn.Sequential(
            nn.Linear(d, e), nn.BatchNorm1d(e), nn.ReLU())
        self.hidden6 = nn.Sequential(
            nn.Linear(e, f), nn.BatchNorm1d(f), nn.ReLU())

        self.regression = nn.Sequential(nn.Linear(f, 1), nn.ReLU())
...
def initialize_mlp(neuron_number_layers,train_loader,learning_rate
    ...
    mlp = MLPmodel(neuron_number_layers[0]
                   ...
                   ,neuron_number_layers[4]
                   ,neuron_number_layers[5]
                  )
    # print(mlp)
    ...
    f"{neuron_number_layers[4]}_{neuron_number_layers[5]}_" + \
    f"{SEED}.pt"
...
def plot_results(train_true_list, train_pred_list, test_true_list, test_pred_list, folder_name):
    ...
    plt.xlabel('DFT Calculated $E_{\mathrm{ads}}$(OH)', fontsize=12)
    plt.ylabel('ML Predicted $E_{\mathrm{ads}}$(OH)', fontsize=12)
...
# Source data
workbook = '8635-Oads.xlsx'  # Input data excel
sheet = str("8635-Oads")  # Input data excel sheet
...
folder_name = f"Source_1_{len(label_s)}_Oads_{label_t_non_null_count}_{len(label_t)}" + \
...
with torch.no_grad():
    h_s_train_initial = mlp.hidden6(mlp.hidden5(mlp.hidden4(mlp.hidden3(
               mlp.hidden2(mlp.hidden1(torch.from_numpy(
               np.array(trainX_s).astype(np.float32)))))))).numpy()
    h_s_test_initial = mlp.hidden6(mlp.hidden5(mlp.hidden4(mlp.hidden3(
              mlp.hidden2(mlp.hidden1(torch.from_numpy(
              np.array(testX_s).astype(np.float32)))))))).numpy()

    h_t_train_initial = mlp.hidden6(mlp.hidden5(mlp.hidden4(mlp.hidden3(
               mlp.hidden2(mlp.hidden1(torch.from_numpy(
               np.array(trainX_t).astype(np.float32)))))))).numpy()
    h_t_test_initial = mlp.hidden6(mlp.hidden5(mlp.hidden4(mlp.hidden3(
              mlp.hidden2(mlp.hidden1(torch.from_numpy(
...
with torch.no_grad():
    h_s = mlp.hidden6(mlp.hidden5(mlp.hidden4(mlp.hidden3(
        mlp.hidden2(mlp.hidden1(torch.from_numpy(
        dataX_source.astype(np.float32)))))))).numpy()

    h_t = mlp.hidden6(mlp.hidden5(mlp.hidden4(mlp.hidden3(
        mlp.hidden2(mlp.hidden1(torch.from_numpy(
